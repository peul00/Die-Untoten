...
public class SceneController : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        SceneManager.activeSceneChanged += OnActiveSceneChanaged; //activeScene이 바뀔 때
        SceneManager.sceneLoaded += OnSceneLoaded; //Scene이 로드 될 떄
        SceneManager.sceneUnloaded += OnSceneUnloaded; // Scene이 언로드 될 때
    }
    ...
    IEnumerator LoadSceneAsync(string sceneName, LoadSceneMode loadSceneMode)
    {
        ...
        while (!asyncOperation.isDone) // 다 로드 되면 isDone을 리턴하고 그전까지는 null을 리턴해서 매 프레임마다 isDone인지 확인
            yield return null;
